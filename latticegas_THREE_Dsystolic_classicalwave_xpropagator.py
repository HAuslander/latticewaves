...
CompZup = 4 # completely arbitrary       
CompZdn = 5 # completely arbitrary       
...
def  ProcessFile():
  ...
  def classicalwavefloatingpt(grid, N_Per_Node=6):
      global NTEST
      # same as above, except that a head-on collision causes outgoing particles to rotate
      # if there's more in one colliding flow than another, the excess will propagate as before  
      xup = np.zeros(grid.shape[:4])
      xup[:, 1:, :, :] = grid[:, :-1, :, :, Comp000]
      xup[:, 0, :, :] = grid[:, -1, :, :, Comp000]
      
      yup = np.zeros(grid.shape[:4])
      yup[:, :, 1:, :] = grid[:, :, :-1, :, Comp090]
      yup[:, :, 0, :] = grid[:, :, -1, :, Comp090]   
      
      zup = np.zeros(grid.shape[:4])
      zup[:, :, :, 1:] = grid[:, :, :, :-1, CompZup]
      zup[:, :, :, 0] = grid[:, :, :, -1, CompZup]   
        
      xdn = np.zeros(grid.shape[:4])
      xdn[:, :-1, :, :] = grid[:, 1:, :, :, Comp180]
      xdn[:, -1, :, :] = grid[:, 0, :, :, Comp180]

      ydn = np.zeros(grid.shape[:4])
      ydn[:, :, :-1, :] = grid[:, :, 1:, :, Comp270]
      ydn[:, :, -1, :] = grid[:, :, 0, :, Comp270]

      zdn = np.zeros(grid.shape[:4])
      zdn[:, :, :, :-1] = grid[:, :, :, 1:, CompZdn]
      zdn[:, :, :, -1] = grid[:, :, :, 0, CompZdn]
      
      a = 2.0 / N_Per_Node # note this is TWICE the "heat-equation" kernel, for which a flow of X on one incoming arc, puts an outgoing flow of X/N_arc for all the outgoing arcs; A in the above equation is always TWICE that.
      ...
      new000 = (xup + yup + xdn + ydn + zup + zdn) * a - xdn
      new090 = (xup + yup + xdn + ydn + zup + zdn) * a - ydn
      new180 = (xup + yup + xdn + ydn + zup + zdn) * a - xup
      new270 = (xup + yup + xdn + ydn + zup + zdn) * a - yup 
      newZup = (xup + yup + xdn + ydn + zup + zdn) * a - zdn
      newZdn = (xup + yup + xdn + ydn + zup + zdn) * a - zup 
      ...
      grid[:, :, :, :, Comp180] = new180  #.astype(int) * randgrid_shuffleprob + xdn * (1-randgrid_shuffleprob)
      grid[:, :, :, :, Comp270] = new270  #.astype(int) * randgrid_shuffleprob + ydn * (1-randgrid_shuffleprob)
      grid[:, :, :, :, CompZup] = newZup  #.astype(int) * randgrid_shuffleprob + xdn * (1-randgrid_shuffleprob)
      grid[:, :, :, :, CompZdn] = newZdn  #.astype(int) * randgrid_shuffleprob + ydn * (1-randgrid_shuffleprob)
  ...
  #INCOMPLETE
  def classicalwave(grid,  gridlatticegas, N_Per_Node=1):
      global NTEST
      # same as above, except that a head-on collision causes outgoing particles to rotate
      # if there's more in one colliding flow than another, the excess will propagate as before  
      xup = np.zeros(grid.shape[:3])
  ...
  def search_nonzero(grid):
    for i in range(grid.shape[1]):
      for j in range(grid.shape[2]):
        for k in range(grid.shape[3]):
          if np.sum(grid[0, i, j, k, :]) != 0:
             print('(', i, j, k, '), ', np.sum(grid[0, i, j, k, :]))
  ...
  NDIM = 128 # must be at least 10 or 20
  history_xval = 0  # this is used to track 1 specific particle
  
  NScenarios = 1  # we get smoother distributions if se generate several different gases and average the amplitube over them, but feel free to set it to 1
  halfband = 0


  grid = np.zeros((NScenarios, NDIM, NDIM, NDIM, 6))
  #gridlatticegas = np.zeros((NScenarios, NDIM, NDIM, NDIM, 6)) # this will be used to feed the random numbers
  ...
  #grid[:, (NDIM//2 - halfband):(NDIM//2 + halfband), (NDIM//2 - halfband):(NDIM//2 + halfband),:] = 512 * 512 * 512 * 512
  
  grid[:,:,:,:,:] = 0
  grid[:, (NDIM//2 - halfband), (NDIM//2 - halfband), (NDIM//2 - halfband), 0] = 1 #512 * 512 * 512 * 512
  print("(", (NDIM//2 - halfband), (NDIM//2 - halfband), (NDIM//2 - halfband), 0, ")")
  ...
  imgplot = plt.imshow(1 - Flatten(grid)).set_cmap('gray')  # show 1st scenario
  plt.axis('off')
  plt.show()
  t=0
  
  
  plt.savefig('C:\\quant\\latgas_soundwave_t_start.jpg')
  ...
  #import glob
  #import os
  #flist = glob.glob('c:\\quant\\wave_movie_stills\\latgas_soundwave_t_*')
  #for ifile in flist:
  #    ifilenew = ifile.replace('latgas_soundwave', 'classwave_pretty')
  #    os.system('copy ' + ifile + ' ' + ifilenew)
  ...
  for t in range(TMAX):
    ...
    if t % 64 == 0: # t % 10 == 0:
      ...
      if bFlatten:
        ...
        if t == 64:
          halfband2 = 4
          stripex = np.sum(grid[0, (NDIM//2 - halfband2):(NDIM//2 + halfband2), :,NDIM//2, :],(0,2))
          stripey = np.sum(grid[0, :, (NDIM//2 - halfband2):(NDIM//2 + halfband2), :, :], (1,2))

          maxval = np.max(np.abs(stripex))
          
          import pdb; pdb.set_trace()
          imggrid = np.sum(grid[0, :, :, NDIM//2,:],2)
          plt.plot(stripex/maxval)
          plt.savefig('C:\\quant\\class3Dwave_just1propagator_' + ('%04d' % (t+1,)) + '.tiff',format='tiff')
          np.savetxt('C:\\quant\\class3Dwave_just1propagator_slicex_' + ('%04d' % (t+1,)) + '.csv', stripex/maxval, fmt='%.6e', delimiter=',', newline='\n')
          np.savetxt('C:\\quant\\class3Dwave_just1propagator_slicey_' + ('%04d' % (t+1,)) + '.csv', stripex/maxval, fmt='%.6e', delimiter=',', newline='\n')
          np.savetxt('C:\\quant\\class3Dwave_just1propagator_fullgrid_' + ('%04d' % (t+1,)) + '.csv', np.sum(grid[0, NDIM//2, :, :, :],2), fmt='%.18e', delimiter='\n', newline='\n')
          plt.show()
          
          
        imgplot = plt.imshow(imggrid).set_cmap('gray')
      ...
      plt.savefig('C:\\quant\\latgas_soundwaveA_t_' + ('%04d' % (t+1,)) + '.jpg',format='jpeg')
