...
be followed at t+1 by a scenario that is also PEP-compliant (after shifting). This technically means the that if a grid point is not PEP-compliant (i.e.
it's sum is > N_arcs) that means that in the earlier time step, there had to be one grid point for which amplitude is > 1, which means it is statistically
sometimes 2 (and this 2 cannot be shifted around, else it would have been).
...
def ReadParams():
  ...
  # THIS VERSION IS DESIGNED TO BE USED IN CONTINUOUS MODE ONLY
...
def ProcessFile():
    ...
    bDiscrete = False # opts.bDiscrete
