...
def  ProcessFile():
  ...
  NDIM = 65 # 101 # must be at least 10 or 20
  ...
  #grid[:, 0, NDIM//2, :] = 1 #  (np.random.random_sample(grid[:, (NDIM//2 - halfband):(NDIM//2 + halfband), (NDIM//2 - halfband):(NDIM//2 + halfband),:].shape) < 0.9) * 512

  grid[:, d2split, :NDIM//2 ,:] = 1 #  
  grid[:, d2split, -NDIM//2: ,:] = -1 #  
  grid[:, d2split, :dA ,:] = 0 #  
  grid[:, d2split, -dA: ,:] = 0 #  
  grid[:, d2split, dA+dB:-(dA+dB) ,:] = 0 #  

  grid[:, d2split + d2screen, : ,:] = 0 #  

  screen = np.zeros((NDIM,))
  ...
  screenplot = plt.plot(screen)  # show 1st scenario
  plt.show()
  ...
  for t in range(TMAX):
    ...
    if True: # t % 10 == 0:
      ...
      if t > 0 and (t % 5000) == 0:
          subscreenfname = 'c:\\quant\\wavescreen_65_30_10_40.pkl'
          if os.path.exists(subscreenfname):
              f = open(subscreenfname, 'rb')
              narr, oldarr = pickle.load(f)
              f.close()
              ...
              oldarr = np.zeros(subscreen.shape)
              narr = 0
          oldarr = oldarr + subscreen
          
          f = open(subscreenfname, 'wb')
          pickle.dump((narr, oldarr), f)
          f.close()  
      ...
      if True: # t % pulsetime == 0:          
          ...
          grid[:, d2split, :NDIM//2 ,:] = randamp(np.cos(2 * np.pi * 2 * t / float(NPer))) # 1 #  
          grid[:, d2split, -NDIM//2: ,:] = randamp(-np.cos(2 * np.pi * 2 * t / float(NPer)))# -1 #  
          grid[:, d2split, :dA ,:] = 0 #  
          grid[:, d2split, -dA: ,:] = 0 #  
          grid[:, d2split, dA+dB:-(dA+dB) ,:] = 0 #  
