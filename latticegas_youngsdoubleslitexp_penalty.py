...
def ReadParams():
  ...
  p.add_option("--wipe", default=False,
                  action="store_true", dest='bWipe', 
                  help="this rezeros the grid every coupld of loops -- makes it easier to see the slit ")


  p.add_option("--addslit", default=False,
                  action="store_true", dest='bAddSlit', 
                  help="this adds the slit")
...
def wavegridupdate(t, grid, gridlatticegas, #fermpenalty, n2penalty, bFermion = False, 
                              Narc=4):
    """
    ...
    for iscen in range(grid.shape[0]):
        for ix in range(grid.shape[1]):
            for iy in range(grid.shape[2]):
                ...
                #if sumamp != 0:
                #    import pdb; pdb.set_trace()
                abssumamp2 = np.abs(sumamp) * 2 # because there are 2 brownian flows for every back-prop flow
                sgnsumamp = np.sign(sumamp)
                
                
                if abssumamp2 >= Narc:
                    allboost = (abssumamp2 // Narc) * sgnsumamp
                    grid[iscen,ix,iy,:] += allboost


                # now randomly draw where to place the rest
                intFlipFac = 1
                modamp = (abssumamp2 % Narc) # remember the factor of 2 is already included in abssumamp
                if modamp > Narc//2: # never true if Narc == 4 but whatever
                    if Narc <= 4:
                      print("how is this even possible -- shouldn't happen")
                    sumamp += Narc * sgnsumamp
                    abssumamp2 += Narc
                    modamp = Narc - modamp
                    intFlipFac = -1
                ...
                # check lattice
                listavail = []
                if modamp > 0 and np.sum(gridlatticegas[iscen,ix,iy,:]) > 0:
                    
                    for i in range(Narc):
                        thislat = gridlatticegas[iscen,ix,iy,i]
                        if thislat != 0:
                            for ipack in range(thislat):
                                listavail.append(i)
                    np.random.shuffle(listavail)
                
                remaining = modamp
                if len(listavail) > 0:
                    remaining = np.max([0, modamp - len(listavail)])
                    howmanyinlist = np.min([modamp, len(listavail)])
                    
                    for idraw in listavail[:howmanyinlist]:
                        thisdraw = idraw
                ...
                # the above has reproduced statistically the wave equation with amplitudes exactly preserved; now, let's assess the errors
                if sumamp != np.sum(grid[iscen,ix,iy,:]):
                    print("outflows do not equal inflows")
                    import pdb; pdb.set_trace()
                
                bRegularize = True
                if bRegularize:
                    a = grid[iscen,ix,iy,0]
                    b = grid[iscen,ix,iy,2]
                    if np.sign(a) != np.sign(b):
                        regdist = np.abs((a-b) // 2)
                        
                        if a < b:
                            grid[iscen,ix,iy,0] += regdist
                            grid[iscen,ix,iy,2] -= regdist
                        else:
                            grid[iscen,ix,iy,2] += regdist
                            grid[iscen,ix,iy,0] -= regdist
                    c = grid[iscen,ix,iy,1]
                    d = grid[iscen,ix,iy,3]
                    if np.sign(c) != np.sign(d):
                        regdist = np.abs((c-d) // 2)
                        
                        if c < d:
                            grid[iscen,ix,iy,1] += regdist
                            grid[iscen,ix,iy,3] -= regdist
                        else:
                            grid[iscen,ix,iy,3] += regdist
                            grid[iscen,ix,iy,1] -= regdist
...
def  ProcessFile():
  ...
  NPer = pulsetime
  bNoWipe = not(opts.bWipe) # disables any grid erasures -- test for amplitube preservation
  if bNoWipe:
      refreshtime = 1000 * 1000 * 1000 * 1000
  else:
      refreshtime = 10*pulsetime # wipe the screen each time
  
  bAddSlit = opts.bAddSlit
  ...
  for t in range(TMAX):
          ...
          if bDiscrete:
              for i in range(grid.shape[0]):
                  ...
                  if t == 0 or (bAddSlit):
                      ...
                      grid[i, d2split, dA ,0] = randamp(grid[i, d2split, dA ,0].shape, np.cos(2 * np.pi * (t % NPer) / float(NPer))) # 1 #  
                      grid[i, d2split, -dA ,0] = randamp(grid[i, d2split, -dA ,0].shape, -np.cos(2 * np.pi * (t % NPer) / float(NPer)))# -1 #  
    ...
    if bDiscrete:
        ...
        if bCplusplus:
          ...
          #import pdb; pdb.set_trace()
          wavegridupdate(t, grid, gridlatticegass, #fermpenalty, n2penalty, bFermion = False, 
                              4)
          #import pdb; pdb.set_trace()
    ...
    print2("sumYmom " + str(np.sum(grid[:,:,:,Comp090] - grid[:,:,:,Comp270])) +  ' max ' + str(np.max(grid)) + ' min ' + str(np.min(grid)) +  ' 90pct ' + str(np.percentile(grid, 90)) + ' min ' + str(np.percentile(grid, 10)) + ' lattsum ' + str(np.sum(gridlatticegass)) + ' min ' + str(np.min(gridlatticegass)) + ' max ' + str(np.max(gridlatticegass))       )
...
#  python latticegas_youngsdoubleslitexp_penalty.py --discrete --seed 222 --picklefile trash  --discrete --seed 222 --addslit