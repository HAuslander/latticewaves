...
So, even though it wasn't updated to reflect as such, we DID find a version that worked (basically
FermiOut6() was the right aproach, I just game up on it too quickly, only to realize that FermiOut8 
would have been the same thing, so we backtracked to 6 and found what seems to be something that works.)

After proving that no PEP-compatible configuration can produce a configuration that is not PEP-compatible
(in the sense that it can't be rectified to be PEP-compatible by way of a heatbath addition of a whorl with
Gaussian-path edges) -- this is done by noting that the underlying equation is linear in the flows along each
arc, and so we just have to prove what we need at the PEP-maxima (i.e. configurations where all the amplitudes are
-1 or 1). We find that the "most problematic" configuration is not where all the amplitudes are the same sign
(that one is suprisingly easy), but rather the one where just one amplitude is -1 and the rest are 1 (or vice versa). For those
configurations the amplitude can be as high as 3-epsilon where epsilon is like 1/N and N is number of arcs. That means that
in the discrete case, there will be some output configurations where there are 3 particles at one arc (so that two of them
will have to be shifted to some other arc that isn't already at PEP-maximum levels)

But it just so conveniently happens that by changing one of the amplitudes to be the opposite sign, we also ensure that 
there are at least two output arc that are zero (which means we'll be able to redirect the PEP) for the two extra
PEP-violators, so we're good.

Now, in order to see how this heat-bath configuration can handle arbitrary amplitudes, we'll START with a configuration
where the 2-D amplitude is 5 at one point (i.e. it must somehow violate PEP since the number of particles there is greater
than the number of arcs), back everything up a few steps, REVERSE the flows, and then do this a bazillion times and verify that
the expected value at this starting point is indeed 5.



NOTE: If this doesn't work, try doing this in a grid where EVERY node is occupied. Then you can spew particle/antiparticle pairs
from two or more nearest neighbor nodes (instead of just trying to control everything by jimmying with the stuff coming out at each node)

But so far, it seems that isn't necessary. It DOES work. Not only is it the case that a PEP-compliant (after shifting) scenario at t will of necessity
be followed at t+1 by a scenario that is also PEP-compliant (after shifting), it is also the cast that if a grid point is not PEP-compliant (i.e.
it's sum is > N_arcs) that means that in the earlier time step, there had to be one neighboring grid point for which amplitude is likewise not PEP-compliant, 
which means it is statistically sometimes 2 (and this 2 cannot be shifted around, else it would have been).

Note that this divide into PEP-compliant and non-PEP-compliant is exhaustive ASSUMING EVERYTHING EVOLVES ACCORDING TO FERMY DYNAMICS (and all amplitudes > 1 are 
shuffled about by way of the heat bath so as to be PEP-compliant). I.e., a PEP-compliant solution that evolves under the
Fermi dynamics as indicated below will stay PEP-compliant, and likewise for non-PEP-compliant solutions. 

But a bunch starting particles whose distributions are all Dirac deltas (and so is PEP-compliant) might become non-PEP-compliant under the Boson dynamics.
""")
...
def SD(x, x2, N):
    if N <= 1.0:
        return 0.0
    sd = (x2 - x * x / float(N)) / np.sqrt(N - 1.0)
    return sd
...
def ReadParams():
  ...
  p.add_option("--picklefile", default="blah.pkl",
  ...
  p.add_option("-t", "--nsteps", "--nstep", default=16,
                  action="store", dest='nsteps', type='int',
                  help="The number of iterations before beginning the test -- should be large enough so that the particle numbers are at their equilibrium values")


  p.add_option("--niter", "--niterations", "--nsamples", default=100,
                  action="store", dest='niter', type='int',
                  help="The number of iterations before beginning the test -- should be large enough so that the particle numbers are at their equilibrium values")
...
def ReverseGrid(grid, grid_br, t):
    """ 
    Take all outgoing flows and transfer them into incoming flows
    (which means they are originating at neighboring nodes oriented in the
    "reverse" direction)
    """

    copygrid = np.copy(grid)
    copygrid_br = np.copy(grid_br)
    grid = grid * 0
    grid_br =  grid_br * 0
    t_out = t - 1
    ...
    for iscen in range(grid.shape[0]):
        for ix in range(grid.shape[1]):
            for iy in range(grid.shape[2]):
                ...
                incarr = copygrid[iscen, ix, iy, :]

                #if np.sum(np.abs([copygrid[iscen, ix, iy, 2],copygrid[iscen, ix, iy, 3],copygrid[iscen, ix, iy, 0],copygrid[iscen, ix, iy, 2]])) > 0:
                #    print("nonzero")
                #    import pdb; pdb.set_trace()
                grid[iscen, ix - 1, iy, 0] = copygrid[iscen, ix, iy, 2]
                grid[iscen, ix, iy - 1, 1] = copygrid[iscen, ix, iy, 3]
                grid[iscen, (ix + 1)%grid.shape[1], iy, 2] = copygrid[iscen, ix, iy, 0]
                grid[iscen, ix, (iy + 1)%grid.shape[2], 3] = copygrid[iscen, ix, iy, 1]

                incarr_br = copygrid_br[iscen, ix, iy, :]
                grid_br[iscen, ix - 1, iy, 0] = copygrid_br[iscen, ix, iy, 2]
                grid_br[iscen, ix, iy - 1, 1] = copygrid_br[iscen, ix, iy, 3]
                grid_br[iscen, (ix + 1)%grid.shape[1], iy, 2] = copygrid_br[iscen, ix, iy, 0]
                grid_br[iscen, ix, (iy + 1)%grid.shape[2], 3] = copygrid_br[iscen, ix, iy, 1]
...
def Initialize5(grid):
    # outward 
    grid[:, 8, 8, :] = 4
    #grid[:, 8, 8, 0] = 5

    grid[:, grid.shape[1]//2 + 8, 8, :] = -4
    #grid[:, grid.shape[1]//2 + 8, 8, 2] = -5



def Initialize4(grid):
    grid[:, 8, 8, :] = 4
    grid[:, grid.shape[1]//2 + 8, grid.shape[2]//2 + 8, :] = -4



def Initialize1(grid):
    aa = grid.shape[1]//2
    bb = grid.shape[2]//4
    #grid[:, bb, bb, :] = 2
    #grid[:, aa + bb, aa + bb, :] = -2
...
def Initialize1only(grid):
    aa = grid.shape[1]//2
    bb = grid.shape[2]//4
    #import pdb; pdb.set_trace()
    grid[:, bb, bb, 0] = 1
    grid[:, aa + bb, aa + bb, 0] = -1

    #import pdb; pdb.set_trace()



def Initialize2only(grid):
    aa = grid.shape[1]//2
    bb = grid.shape[2]//4
    #import pdb; pdb.set_trace()
    grid[:, bb, bb, 0] = 1
    grid[:, bb, bb, 1] = -1
    grid[:, aa + bb, aa + bb, 2] = -1
    grid[:, aa + bb, aa + bb, 3] = 1

    #import pdb; pdb.set_trace()



def Initializemany(grid):
    aa = grid.shape[1]//2
    bb = grid.shape[2]//4
    for i in range(grid.shape[1] // 2):
        for j in range(grid.shape[2] // 2):
            if (i + j + 0) % 2 != 1:
                continue # wrong parity  
            k = rn.choice(np.arange(2*grid.shape[-1]))
            bnegate = False
            #if k >= grid.shape[-1]:
            #    bnegate = True
            k = (k % grid.shape[-1])
            thisone = -1 if bnegate else 1
            grid[0, i, j, k] = thisone
            grid[0, i + aa, j, k] = -thisone
            grid[0, i, j + bb, k] = -thisone
            grid[0, i + aa, j + bb, k] = thisone
...
def InitializeRand(grid, NPairs):
    # outward 
    def Getxy(i, grid):
        x = i // grid.shape[2]
        y = i % grid.shape[1]
    ...
    indices = rn.choice((grid.shape[1] * grid.shape[2])//2, NPairs * 2, False)
...
https://old.reddit.com/r/news/comments/1n1iiyo/minnesota_governor_says_a_shooting_has_occurred/
    ...
    def WhicheverWorks(err, modpoolout, signsumerr):
        ...
        if tuple(err) == (0, -2,  1,  0) and tuple(modpoolout) == (-1, -1, -1, -1) and signsumerr == -1:
            print("b")
            import pdb; pdb.set_trace()
    ...
    if sumerr == 0:
        if bPrint:
            print("out", inputarcarr, modpoolout, err) 
    ...
    if bPrint:
        print("out", inputarcarr, origmodpoolout, modpoolout, err)
    return modpoolout, err
...
def wavegridupdate(t, grid, grid_br): # fermpenalty, n2penalty, bFermion = False, Narc=4):
    ...
    def ModPoolJustBrownianOLD(inlist):
    ...
    for iscen in range(grid.shape[0]):
        for ix in range(grid.shape[1]):
            for iy in range(grid.shape[2]):
                ...
                if False and t < TMAX//2: 
                    #bosebrownhuygout, displaced = BoseOut(incarr)
                    outarr = ModPool(incarr)

                    if np.sum(outarr) != np.sum(incarr):
                        print("wrong")
                        import pdb; pdb.set_trae()
                    outgoing_inc_heat_bath = ShuffleInToOut(outarr)
                    grid[iscen, ix, iy, :] = outgoing_inc_heat_bath
                    #displaced = [0] * len(bosebrownhuygout)
                #    bosebrownhuygout, displaced = BoseOut(incarr)

                    

                elif bFermi: #bFermi and not(t < TMAX//2):
                    ...
                    #if np.sum(np.abs(incarr_br)) > 0 and np.sum(np.abs(incarr)) > 0:
                        
                    #    import pdb; pdb.set_trace()

                    forward_emission_heat_bath_contribution = ModPoolJustBrownian(incarr_br) 

                    #if (np.min(forward_emission_heat_bath_contribution) < np.min(incarr_br)) or (np.max(forward_emission_heat_bath_contribution) < np.max(incarr_br)):
                    #    import pdb; pdb.set_trace()

                    reversed_emission_heat_bath_contribution = -np.array(incarr_br).astype("int")

                    from_heat_bath = forward_emission_heat_bath_contribution + reversed_emission_heat_bath_contribution
                    #if np.sum(np.abs(from_heat_bath)) > 2:
                    #    import pdb; pdb.set_trace()

                    incarr_wheatbath = incarr + from_heat_bath
                    #if np.min(incarr_wheatbath) * np.max(incarr_wheatbath) < 0:
                    #    import pdb; pdb.set_trace()
                    fermibrownhuygout, displaced = FermiOut6(incarr + from_heat_bath) 
                    

                    grid[iscen, ix, iy, :] = ShuffleInToOut(fermibrownhuygout)
                    grid_br[iscen, ix, iy, :] = ShuffleInToOut(forward_emission_heat_bath_contribution + displaced)

                    outgoing_inc_heat_bath = grid[iscen, ix, iy, :]

                    if np.sum(outgoing_inc_heat_bath) > 4:
                        print("Trouble")
                        import pdb; pdb.set_trace()
                    ...
                    bFinalFlatten = False
                    if bFinalFlatten:
                        # in this version, you flatten the heat bath by spewing particle/antiparticle pairs to even out the total outflow -- you STILL get PEP violations in grid_br
                        grid_br[iscen, ix, iy, :] = ShuffleInToOut(ModPoolJustBrownian(forward_emission_heat_bath_contribution + xtradisplaced))
                    else:
                        # in this version, the heatbath can have large amplitudes in some arcs (in 2-d they can be as high as about 8), and  yet
                        # the PEP for the regular grid is still preserved, and the overall particle count still stays bounded.
                        grid_br[iscen, ix, iy, :] = ShuffleInToOut(forward_emission_heat_bath_contribution + xtradisplaced)

                    if np.max(np.abs(grid[iscen, ix, iy, :])) > 1:
                        print("ERRinout", grid_br[iscen, ix, iy, :])
                        import pdb; pdb.set_trace()

                    #if np.abs(np.sum(grid_br[iscen, ix, iy, :])) > 4:
                    #    print("ERRRRRRRERRinheatbath", grid[iscen, ix, iy, :], grid_br[iscen, ix, iy, :])
                        #import pdb; pdb.set_trace()
                    """
    ...
    #import pdb; pdb.set_trace()
    if False: #bBrownianRegularization:
        if t % 2 == 0:
            grid_br[:, 0::2, 0::2, :] = 0
            grid_br[:, 1::2, 1::2, :] = 0
        else:
...
def ProcessFile():
    ...
    NDIM = 64 # 128 #64 # 32 # 64 # 4 # 64 # 100 # must be at least 10 or 20 
    ...
    NScenarios = 1 # 2 # we get smoother distributions if se generate several different gases and average the amplitube over them
    ...
    gridNerr = 0
    gridsumerr = np.zeros(grid.shape[:-1])
    gridsumerrsq = np.zeros(grid.shape[:-1])


    Ngridaccrue = 0
    sumgridaccrue = np.zeros(grid.shape)
    sumgridaccrue_br = np.zeros(grid.shape)    
    sumgridaccrue_both = np.zeros(grid.shape)    
    sumgridaccrue_diff = np.zeros(grid.shape)    
    ...
    TMAX = opts.nsteps # max([50 * 1000 * 1000, nplotmod])
    ...
    for irun in range(NRuns):
        for t in range(TMAX):
            ...
            #import pdb; pdb.set_trace()
            if False: # and t == TMAX//2:
                #print("ALERT: at t=", t, " it is time to reverse")
                #import pdb; pdb.set_trace()


                (grid, grid_brown) = ReverseGrid(grid, grid_brown, t)
        ...
        #import pdb; pdb.set_trace()
        sumgridaccrue = sumgridaccrue + grid
        sumgridaccrue_br = sumgridaccrue_br + grid_brown
        sumgridaccrue_both = sumgridaccrue_both + grid + grid_brown
        sumgridaccrue_diff = sumgridaccrue_both + grid - grid_brown
        tgt1716.append(grid[0,  17, 16, :])
        tgt4948.append(grid[0, 49, 48, :])
        Ngridaccrue += 1
    ...
    import pprint
    ####### PRINT OUT RESULTS  -- should be similar to initial conditions

    print("ALERT: Now print out results -- should be similar to initial conditions")
    import pdb; pdb.set_trace()
    pprint(sumgridaccrue/float(Ngridaccrue))
...
if __name__ == '__main__':   
    ...
    """
    0) select if it's Boson or Fermion

    Loop many times:

        1) define grid and initialize it using random seed ()        

        2) iterate wave for some number of steps to make grid_br "representative"

        3) save grid as gridtM2, then iterate and save grid as gridtM1, then iterate CLASSICAL wave on the final configuration as gridcont

        Loop many times:
            4) iterate gridtM1 once

            5) accrue the error (differences with gridcont)
    """
...
%run  latticegas_youngsdoubleslitexp_modpool_FERMIattmpt7.py   --discrete --seed 45 --picklefile trash3.pkl --alpha 1.0


%run latticegas_youngsdoubleslitexp_modpool_FERMIattmpt7_testwavefunction.py --discrete -t 4
"""