...
def  ProcessFile():
  ...
  #INCOMPLETE
  def classicalwave(grid,  i, j, N_Per_Arc=1):
      global NTEST
      # same as above, except that a head-on collision causes outgoing particles to rotate
      # if there's more in one colliding flow than another, the excess will propagate as before  
      xup = grid[:, i-1,j,Comp000] # note the name xup indicates the flow is coming FROM the xDOWN direction (and heading in the xup direction)
      yup = grid[:, i,j-1,Comp090]
      xdn = grid[:, (i+1) % grid.shape[1],j,Comp180]
      ydn = grid[:, i,(j+1) % grid.shape[2],Comp270]
  ...
  NDIM = 4 # must be at least 10 or 20
  ...
  grid[:, NDIM//2, NDIM//2, 0] = 1
  #classicalwavefloatingpt(grid)
  #
  print("eh0", grid[0, NDIM//2+1, NDIM//2, 0] )
  print("eh1", grid[0, NDIM//2+1, NDIM//2, 1] )
  print("eh2", grid[0, NDIM//2+1, NDIM//2, 2] )
  print("eh3", grid[0, NDIM//2+1, NDIM//2, 3] )
  #
  #classicalwavefloatingpt(grid)
  #
  #print("eh0", grid[0, NDIM//2+1, NDIM//2, 0] )
  ...
  #classicalwavefloatingpt(grid)
  #
  #print("eh0", grid[0, 2, 1, 0] )
  #print("eh1", grid[0, 2, 1, 1] )
  #print("eh2", grid[0, 2, 1, 2] )
  #print("eh3", grid[0, 2, 1, 3] )
  #
  #classicalwavefloatingpt(grid)
  #
  #print("eh0", grid[0, 2, 1, 0] )
  ...
  #grid[:,:,:,:] = np.random.random_sample(grid[:,:,:,:].shape) < 0.4
  ...
  def checkdir(prevgrid, currgrid, direction):
      # if direction==Comp000 correlates all outgoing components (which are always orderedstartingwithComp000)
      
      mean_indir_outdirs = np.zeros((4,))
      ...
      for idir in range(4):
          ...
          for i in range(1, NDIM-1):
              for j in range(1, NDIM-1):
                  for iscen in range(prevgrid.shape[0]):
                      prevval = prevgrid[iscen, i, j, direction]
                      
                      if False:#prevval == 0:
                          continue
                      else:
                          if direction == Comp000:
                              currval = currgrid[iscen, i+1, j, idir]# / float(prevval)
                          elif direction == Comp090:
                              currval = currgrid[iscen, i, j+1, idir]# / float(prevval)
                          elif direction == Comp180:
                              currval = currgrid[iscen, i-1, j, idir]# / float(prevval)
                          elif direction == Comp270:
                              currval = currgrid[iscen, i, j-1, idir]# / float(prevval)
                          N += 1
                          SumX += prevval
                          SumY += currval
                          SumXY += prevval * currval
                          SumX2 += prevval * prevval
                          SumY2 += currval * currval
                          
                          stdx = np.sqrt((SumX2 - SumX * SumX / float(N)) / float(N-1))
                          stdy = np.sqrt((SumY2 - SumY * SumY / float(N)) / float(N-1))
                          Covar = (SumXY - SumX * SumY / float(N)) / float(N-1)
                          
                          
                          
          #
          #idir = 0s
          mean_indir_outdirs[idir] = Covar / np.sqrt(stdx * stdy)
  
          print("coeff", direction, "_", idir, " ", mean_indir_outdirs[idir])
          import pdb; pdb.set_trace()
  ...
  for t in range(TMAX):
    ...
    #propagate_rotate90oncollision_ONESWEEP(grid)
    ...
    if bCheckCoeff:
        if (t+1) % NCheckCoeff == 0:
            #import pdb; pdb.set_trace()
            prev = copy.copy(grid)
        elif t != 0 and t % NCheckCoeff == 0:
            import pdb; pdb.set_trace()
            checkdir(prev, grid, Comp000)
            checkdir(prev, grid, Comp090)
            checkdir(prev, grid, Comp180)
            checkdir(prev, grid, Comp270)
            import pdb; pdb.set_trace()
