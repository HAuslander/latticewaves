...
def  ProcessFile():
  ...
  def classicalwavefloatingpt(grid, N_Per_Node=4):
      global NTEST
      # same as above, except that a head-on collision causes outgoing particles to rotate
      # if there's more in one colliding flow than another, the excess will propagate as before  
      xup = np.zeros(grid.shape[:3])
  ...
  #INCOMPLETE
  def classicalwave(grid,  gridlatticegas, N_Per_Node=4):
      global NTEST
      # same as above, except that a head-on collision causes outgoing particles to rotate
      # if there's more in one colliding flow than another, the excess will propagate as before  
      xup = np.zeros(grid.shape[:3])
  ...
  NDIM = 128 # 256 # must be at least 10 or 20
  ...
  grid[:,:,:,:] = 0
  grid[:, (NDIM//2 - halfband), (NDIM//2 - halfband),1] = 512 * 512 * 512 * 512
  ...
  for t in range(TMAX):
    ...
    if t % 64 == 0: # t % 10 == 0:
      ...
      if bFlatten:
        ...
        if t == 64:
          halfband2 = 4
          stripex = np.sum(grid[0, (NDIM//2 - halfband2):(NDIM//2 + halfband2), :, :],(0,2))
          stripey = np.sum(grid[0, :, (NDIM//2 - halfband2):(NDIM//2 + halfband2), :],(1,2))
          maxval = np.max(np.abs(stripex))
          
          import pdb; pdb.set_trace()
          imggrid = np.sum(grid[0, :, :,:],2)
          
          
          plt.savefig('C:\\quant\\class2Dwave_just1propagator_' + ('%04d' % (t+1,)) + '.png',format='png')
          np.savetxt('C:\\quant\\class2Dwave_just1propagator_fullgrid_' + ('%04d' % (t+1,)) + '.csv', np.sum(grid[0, :, :,:],2), fmt='%.6e', delimiter=',', newline='\n')
          np.savetxt('C:\\quant\\class2Dwave_just1propagator_slicey_' + ('%04d' % (t+1,)) + '.csv', stripey/maxval, fmt='%.18e', delimiter='\n', newline='\n')
          np.savetxt('C:\\quant\\class2Dwave_just1propagator_slicex_' + ('%04d' % (t+1,)) + '.csv', stripex/maxval, fmt='%.18e', delimiter='\n', newline='\n')
          plt.plot(stripex/maxval)
          plt.show()
          plt.plot(stripey/maxval)
          plt.show()
      ...
      plt.savefig('C:\\quant\\latgas_soundwaveBW_t_' + ('%04d' % (t+1,)) + '.png',format='png')
